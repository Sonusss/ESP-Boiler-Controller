...
const char* VERS = "ESP_Boiler_Controller_01.02";
...
void callback_MQTT(char* topic, byte* payload, unsigned int length) {
  ...
  JsonVariant Jboiler_active = doc0["boiler_active"];  
  JsonVariant Jheating_temp = doc0["heating_temp"];  //debug purpose
  JsonVariant Jreturn_temp = doc0["return_temp"];  //debug purpose
  JsonVariant Jboiler_temp = doc0["boiler_temp"]; //debug purpose
...
//  if (!Jprobe.isNull()) { publish_MQTT_sensors(); };
  ...
  if (!Jboiler_active.isNull()) { 
    if (Jboiler_active == 1) { boiler_active = true; };
    if (Jboiler_active == 0) { boiler_active = false; };
  }  
...
void publish_MQTT_stat() {
  ...
  status0["boiler_active"] = String(boiler_active);
...
void boiler_rules(){
  if (boiler == 1) {
    ...
    if (heating_temp < boiler_heating_temp -boiler_temp_bias && boiler_active == true && burner == false) {
      //relay(2,"on");
      c_burner = true;
    }
    ...
    if (boiler_temp < heating_temp && heating_temp > boiler_temp + boiler_temp_bias && boiler_temp < boiler_preset_temp && boiler_active == true && b_pump == false) {
      relay(4, "on");
    }
  }
  if (boiler == 0) {
